...
cv_bridge::CvImagePtr cv_ptr_drawn = null;
//raw image
void MultiAprilTagsTracker::imageCallback( const sensor_msgs::ImageConstPtr& msg) {
  ...
  if( tags.size() > 0 && cv_ptr_drawn != nullptr) {
    multi_apriltags_tracker::april_tag_pos msg;
    for( unsigned int i=0; i<tags.size(); i++ ){
      AprilTags::TagDetection tag = tags[i];
      circle( cv_ptr_drawn->image, Point2f( tag.cxy.first, tag.cxy.second ), 2, Scalar(0,255,0), 4 );
      line( cv_ptr_drawn->image, Point( tag.p[0].first, tag.p[0].second ), Point( tag.p[1].first, tag.p[1].second ), Scalar(0,255,0), 2 );
      line( cv_ptr_drawn->image, Point( tag.p[1].first, tag.p[1].second ), Point( tag.p[2].first, tag.p[2].second ), Scalar(0,255,0), 2 );
      line( cv_ptr_drawn->image, Point( tag.p[2].first, tag.p[2].second ), Point( tag.p[3].first, tag.p[3].second ), Scalar(0,255,0), 2 );
      line( cv_ptr_drawn->image, Point( tag.p[3].first, tag.p[3].second ), Point( tag.p[0].first, tag.p[0].second ), Scalar(0,255,0), 2 );
      double orientation_length = sqrt( pow(tag.p[0].first-tag.p[1].first,2) + pow(tag.p[0].second-tag.p[1].second,2) );
      line( cv_ptr_drawn->image, Point( tag.cxy.first, tag.cxy.second ), Point( tag.cxy.first+orientation_length*cos(tag.getXYOrientation()), tag.cxy.second+orientation_length*sin(tag.getXYOrientation()) ), Scalar(0,255,0), 2 );
  ...
  cv::imshow(MULTI_APRIL_TAGS_TRACKER_VIEW, cv_ptr_drawn->image );
  /*
...
//drawn image
void MultiAprilTagsTracker::imageCallbackTwo( const sensor_msgs::ImageConstPtr& msg) {
  //cv_bridge::CvImagePtr cv_ptr;
  try {
    cv_ptr_drawn = cv_bridge::toCvCopy( msg, "bgr8" );
  }
  catch( cv_bridge::Exception& e ) {
    ROS_ERROR( "cv_bridge exception: %s", e.what() );
    return;
...
MultiAprilTagsTracker::MultiAprilTagsTracker( AprilTags::TagCodes codes  ) : m_it( m_nh ) , m_tag_codes( codes )  {
  ...
  m_sub_two = m_it.subscribe("/output/image_drawn", 1, &MultiAprilTagsTracker::imageCallbackTwo, this);
